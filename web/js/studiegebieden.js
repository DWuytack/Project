...
/*
 function toggleModules(OpleidingBtn) {
 if (OpleidingBtn.textContent === "+") {   // LAAD ALLE OPLEIDINGEN IN TABEL
 ...
 //open(method,url,async)
 var OpleidingID = OpleidingBtn.parentElement.parentElement.dataset.id;
 xhttp.open("POST", "StudiegebiedenServlet?opleiding=" + OpleidingID, true);
 xhttp.send();
 xhttp.onreadystatechange = function () {
 ...
 button = document.createElement('button');
 button.setAttribute("onclick", "toggleDoelstellingen(this)");
 button.textContent = "+";
 ...
 OpleidingBtn.textContent = "-";
 OpleidingBtn.parentElement.appendChild(table);
 }
 };
 } else if (OpleidingBtn.textContent === "-") {
 OpleidingBtn.parentElement.removeChild(OpleidingBtn.parentElement.childNodes[2]);
 OpleidingBtn.textContent = "+";
 }
 }
 */